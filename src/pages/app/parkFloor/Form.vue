  ...
  <div>
    <el-form :model="entity" :rules="rules" ref="form" label-width="100px">
        <el-form-item label="排序" prop="seq">
            <el-input v-model="entity.seq" placeholder="不填写则由系统生成"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate((valid, err) => {
        if (valid) {
          this.$api.ipark.ParkFloorService.save(this.entity).then(rsp => {
            this.$emit("saved", rsp);
          });
        }
      });
    ...
    init(options) {
      ...
      if (options.id) {
        //loadData
        this.$api.ipark.ParkFloorService.get(options.id).then(r => {
          this.entity = r;
        });
      }
    }
