...
　　      <span v-else></span>
      ...
      <el-table-column prop="pressure" label="电量" width="60">
        <template slot-scope="{row}">{{(row.parkPort||{}).electricQuantity}}</template>
      </el-table-column> 
      
      <el-table-column prop="status" width="80" label="订单状态">
        <template slot-scope="{row}">
          <span v-if='row.status===0'>已取消</span>
　　      <span v-else-if='row.status===1'>待付款</span>
          <span v-else-if='row.status===2'>已付款</span>
          <span v-else-if='row.status===3'>未履约</span>
          <span v-else-if='row.status===4'>履约中</span>
          <span v-else-if='row.status===5'>已完结</span>
　　      <span v-else></span>
        </template>
      </el-table-column>
...
　　      <span v-else></span>
      ...
      <el-table-column prop="version" label="地锁版本型号" width="80">
...
export default {
  ...
  data() {
    return {
      ...
      page: {
        ...
        getData: this.$api.ipark.BookingOrderService.queryorder
      },
      tableActions: [
        {
          name: "编辑",
  ...
  methods: {
    ...
    share(row) {
      console.log(row)
      this.$refs.formDiag.show({ orderNo: row.orderNo }); 
      this.shares=false; 
      this.disable=true    
    },
    edit(row) {
